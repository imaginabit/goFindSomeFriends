...
window.onload = function () {
	...
	*/
	...
	Crafty.scene("loading", function (){
		...
		//console.log(h/5);
		Crafty.e("2D, DOM, Text").attr({ w: w, h: 20, x: 0, y: Math.round((h/5)*4)	})
				.text("Presents")
				.css({ "text-align": "center" });

		Crafty.e("2D, DOM, Image").image("logo.png")
		.attr({w:256,h:256,x: w/2-256, y:-30});
	...
	Crafty.c("friendA",{
		...
		follow: function(obj){
			...
			if (!this.isFollow && typeof(obj) != "undefined"){
				this.isFollow=true;
				obj.friends+=1;
				obj.mood_up();
				this.bind("EnterFrame",function(){
					if (time+500<Date.now()){
						...
						velocidad= Crafty.randRange(1,10);
    ...
    Crafty.scene("game",function(){
		...
		*/
		...
		Crafty.bind("EnterFrame",function(){
			...
			if (time+1000<Date.now() && !stop){
				//console.log(stop);
				//Crafty.randRange(0,texts.length-1)
				...
				if (!player.die){
					score+=1;
					player.faceChange(Math.floor(5*player.status.w/player.barra.w));
					time= Date.now();
				} else {
					player.face.destroy();
					player.destroy();
    ...
    Crafty.c('moodFace',{
    	init: function(){
		    ...
		    //this.requires("")
		   	...
		   	this.cara=2;
    		this.face= Crafty.e("2D,DOM, SpriteAnimation, mood"+this.cara ).attr(at);
...
/*
    	...
    	faceChange: function(n) {
    		...
    		if (n!=this.cara){
    			this.face.removeComponent("mood0");
    			this.face.removeComponent("mood1");
    			this.face.removeComponent("mood3");
    			this.face.removeComponent("mood4");
    			if (n!=2)this.face.addComponent("mood"+n);
    			//console.log("faceChange "+n);
    			this.cara=n;
			}
    	}
